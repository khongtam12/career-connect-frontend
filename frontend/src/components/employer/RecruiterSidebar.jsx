...
const RecruiterSidebar = ({ isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    { name: 'Tổng quan', icon: <FiHome />, path: '/employer' },
    { name: 'Quản lý tin đăng', icon: <FiBriefcase />, path: '/employer/jobs' },
    { name: 'Hồ sơ ứng viên', icon: <FiUsers />, path: '/employer/candidates', badge: '12' },
    { name: 'Lịch phỏng vấn', icon: <FiCalendar />, path: '/employer/interviews' },
    { name: 'Báo cáo hiệu quả', icon: <FiPieChart />, path: '/employer/reports' },
    { name: 'Gói dịch vụ', icon: <FiCreditCard />, path: '/employer/billing' },
    { name: 'Cài đặt tài khoản', icon: <FiSettings />, path: '/employer/settings' },
  ];
          ...
          {menuItems.map((item, index) => {
            ...
            return (
              <NavLink
                ...
                className={`flex items-center justify-between px-4 py-2.5 text-sm font-medium rounded-lg transition-colors no-underline ${active
